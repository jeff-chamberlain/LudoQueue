...
exports.config_game = function(socket) {
	if(game_socket == null) {
		...
		socket.on('game_start', function(new_state) {
			all_players.emit('game_start');
		});
		
		socket.on('game_end', function(winner) {
			all_players.emit('game_end',winner);
		});
		...
		socket.on('spill', function(id) {
			if(player_sockets.hasOwnProperty(id)) {
				player_sockets[id].emit('spill');
			}
		});
